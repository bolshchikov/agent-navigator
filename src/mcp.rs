...
impl AgentNavigatorMcp {
    ...
    async fn navigate(&self, Parameters(params): Parameters<NavigateParams>) -> String {
        encode("navigate", self.inner.navigate(to_nav(params)).await)
    }

    #[tool(
        description = "Fetch a URL and return extracted markdown plus metadata. Same envelope as navigate."
    )]
    async fn extract(&self, Parameters(params): Parameters<NavigateParams>) -> String {
        encode("extract", self.inner.extract(to_nav(params)).await)
    }
    ...
    async fn discover(&self, Parameters(params): Parameters<NavigateParams>) -> String {
        encode("discover", self.inner.discover(to_nav(params)).await)
    }
    ...
    async fn call_webmcp_tool(&self, Parameters(params): Parameters<CallToolParams>) -> String {
        encode(
            "call_webmcp_tool",
            self.inner
                .call_webmcp_tool(
                    ...
                    params.session,
                    params.ignore_robots.unwrap_or(false),
                )
                .await,
        )
    }
    ...
    async fn submit_form(&self, Parameters(params): Parameters<SubmitFormParams>) -> String {
        encode(
            "submit_form",
            self.inner
                .submit_form(
...
robots.txt is respected unless ignore_robots is explicitly true.",
            )
    }
    ...
    async fn call_tool(
        ...
        tracing::info!(
            ...
            "MCP tools/call — client invoked an AgentNavigator tool over stdio JSON-RPC"
        );
    ...
    async fn on_initialized(&self, _context: rmcp::service::NotificationContext<rmcp::RoleServer>) {
        tracing::info!(
            "MCP client initialized over stdio. Declarative WebMCP on a page is HTML <form toolname tooldescription>; calling it uses call_webmcp_tool, which issues a normal HTTP request — the website is never spoken to with MCP JSON-RPC."
        );
    }
}

fn to_nav(params: NavigateParams) -> NavigateRequest {
    NavigateRequest {
        url: params.url,
        session: params.session,
        include_html: params.include_html.unwrap_or(false),
